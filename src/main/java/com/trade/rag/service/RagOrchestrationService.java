...
import com.trade.shared.dto.Citation;
...
public class RagOrchestrationService {
    ...
    public RagOrchestrationService(
            ...
            AiRequestMapper.ChatOrchestrationService chatService,
                ...
                .switchIfEmpty(
                        Flux.defer(() -> {
                            ...
                            return hybridSearchService.search(question, maxResults)
                                        ...
                                        return rerankService.rerank(question, matches)
                                                .flatMapMany(reranked -> {
                                                    ...
                                                    if (reranked.isEmpty() || reranked.get(0).score() < 0.6) {
                                                        ...
                                                        Flux<AiStreamEvent> noAnswerFlux = Flux.concat(
                                    ...
                                    .doOnNext(event -> {
                                        if (event.type() == com.trade.shared.enums.StreamEventType.TOKEN && event.content() != null) {
                                            answerBuf[0].append(event.content());
                                        }
                                        if (event.type() == com.trade.shared.enums.StreamEventType.DONE) {

...
import com.trade.shared.exception.VectorStoreException;
import com.trade.rag.dto.DocumentUploadRequest;
import com.trade.rag.dto.DocumentUploadResponse;
import com.trade.conversation.service.MinioStorageService;
...
public class DocumentIngestionService {
     ...
     * @return 批量摄入结果
     */
    public Mono<BatchUploadResponse> ingestFromFiles(
            List<FilePart> files,
            String titlePrefix,
            String metadataJson
    ) {
        Map<String, Object> metadata = parseMetadata(metadataJson);
        List<Mono<BatchUploadResponse.FileUploadResult>> monos = files.stream()
                .map(file -> {
                    ...
                    return ingestFromFile(file, request)
                            .map(resp -> new BatchUploadResponse.FileUploadResult(
                                    file.filename(),
                                    resp.documentId(),
                                    resp.chunkCount(),
                                    "success",
                            ...
                            .onErrorResume(e -> Mono.just(
                                    new BatchUploadResponse.FileUploadResult(
                                            file.filename(),
                                            null,
                                            0,
                                            "failed",
        ...
        return Mono.zip(monos, results -> {
            List<BatchUploadResponse.FileUploadResult> fileResults = new ArrayList<>();
            int successCount = 0;
            int failedCount = 0;
            for (Object obj : results) {
                BatchUploadResponse.FileUploadResult r =
                        (BatchUploadResponse.FileUploadResult) obj;
                fileResults.add(r);

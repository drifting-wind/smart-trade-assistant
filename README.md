...
### 安全配置
- `ai.gateway.security.sensitive-word.enabled`: 是否启用敏感词过滤（默认 true）
- `ai.gateway.security.prompt-injection.enabled`: 是否启用 Prompt 注入防护（默认 true）
- `ai.gateway.security.xss.enabled`: 是否启用 XSS 防护（默认 true）
- `ai.gateway.security.xss.block-mode`: XSS 防护模式，true 拦截可疑请求，false 清洗可疑内容（默认 false）
...
详细文档请参考 [docs/SECURITY.md](docs/SECURITY.md) 
